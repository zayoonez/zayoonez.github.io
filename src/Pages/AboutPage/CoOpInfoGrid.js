...
export default function CoOpInfoGrid() {
  ...
  useEffect(() => {
    axios
      .get(
        "https://port-0-promoationpage-server-12fhqa2blnlum4de.sel5.cloudtype.app/api/partners"
      )
  ...
  return (
    <BoxContainer>
        ...
        {mainData.map((item) => (
          <MainLogo>
            <Img
              ...
              alt="logo image"
              onClick={() => goManagement(item.link)}
            />
          </MainLogo>
        ))}
      ...
      >
        {subData.map((item) => (
          <SubLogo>
            <Img src={item.logoImageUrl} alt="logo image" />
          </SubLogo>
        ))}
      </SubLogoDiv>
      {/*<TableWidth*/}
